...
class StepTrade(Strategy):
    def __init__(
        ...
        result = self.db_handler.find_item(
            {"name": strategy},
            "trader",
            "trade_strategy",
        )
        ...
        if self.token is None:
            ...
            saved_refresh_token = self.redis.get(
                str(self.machine) + self.machine.get_username() + "refresh",
            )
            if saved_refresh_token is None:
                ...
                expire, token, refresh = self.machine.set_token(
                    grant_type="refresh_token",
                )
            self.redis.set(
                str(self.machine) + self.machine.get_username(),
                token,
                expire,
            )
            self.redis.set(
    ...
    def scenario(self):
        ...
        pipeline_5m = [
            {
                "$match": {
                    "timestamp": {"$gt": five_min_ago_timestamp},
                    "coin": self.currency_type,
                },
            },
            {
                "$group": {
        ...
        my_orders = self.db_handler.find_items(
            {
                ...
                "buy": {
                    "$gte": self.last_val - int(self.params["step_value"]),
                    "$lte": self.last_val + int(self.params["step_value"]),
                },
            },
            "trader",
            "trade_status",
        ...
        if my_orders.count() > 0:
            ...
            self.item = {
                "buy": str(self.last_val),
                "buy_amount": self.params["buy_amount"],
                "currency": self.currency_type,
            }
            self.item["desired_value"] = int(
                self.last_val + int(self.params["target_profit"]) / float(self.params["buy_amount"]),
            )
            logger.info(self.item)
            wallet = self.machine.get_wallet_status()
            if int(wallet["krw"]["avail"]) > int(
                self.last_val * float(self.params["buy_amount"]),
            ):
                self.order_buy_transaction(
    ...
    def check_buy_ordered(self):
        buy_orders = self.db_handler.find_items(
            {"currency": self.currency_type, "status": "BUY_ORDERED"},
            "trader",
            "trade_status",
        )
        ...
        for item in buy_orders:
            logger.info(item)
            order_result = self.machine.get_my_order_status(
                self.currency_type,
                order_id=item["buy_order_id"],
            )
            logger.info(order_result)
            if (
                len(order_result) > 0
                and order_result[0]["status"] == "filled"
                and order_result[0]["price"] == str(item["buy"])
            ):
                order_result_dict = order_result[0]
                real_buy_amount = float(order_result_dict["filled_amount"]) - float(
                    order_result_dict["fee"],
                )
                ...
                len(order_result) > 0
                and order_result[0]["status"] == "unfilled"
                and order_result[0]["price"] == str(item["buy"])
            ):
    ...
    def check_buy_completed(self):
        buy_completed = self.db_handler.find_items(
            {"currency": self.currency_type, "status": "BUY_COMPLETED"},
            "trader",
            "trade_status",
        )
    ...
    def check_sell_ordered(self):
        sell_orders = self.db_handler.find_items(
            {"currency": self.currency_type, "status": "SELL_ORDERED"},
            "trader",
            "trade_status",
        )
        for item in sell_orders:
            ...
            if (
                len(order_result) > 0
                and order_result[0]["status"] == "filled"
                and order_result[0]["price"] == str(item["desired_value"])
            ):
                order_result_dict = order_result[0]
    ...
    def check_sell_completed(self):
        sell_completed = self.db_handler.find_items(
            {
                "currency": self.currency_type,
    ...
    def check_keep_ordered(self):
        keeped_orders = self.db_handler.find_items(
            {"currency": self.currency_type, "status": "KEEP_ORDERED"},
            "trader",
            "trade_status",
        )
